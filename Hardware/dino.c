...
void Show_Dino(void){
	...
	dino.maxY=64-Jump_Pos;
}

int isColling(struct Object_Position *a,struct Object_Position *b){
...
int DinoGame_Animation(void){
	...
	for(;;){
		...
		return_flag=isColling(&dino,&barrier);
		if(return_flag==1){
			return 0;
		}
...
void DinoGame_Pos_Init(void){
	...
	Score=Ground_Pos=Barrier_Pos=Jump_Pos=0;
